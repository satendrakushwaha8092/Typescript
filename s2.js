...
const sourceWorkbookPath = "../CJ Labor Standard  food cost 2023-08.xlsx"; // Replace with the path to your source Excel file
const pnlWorkbookPath = "../FF IS - CJSC - 08.23.xlsx";
const salesWorkbookPath = "../All live GL 2023.xlsx";
const targetWorkbookPath = "../target3.xlsx"; // Replace with the path to your target Excel file
...
const date = "18-08-2023"; //all dates depend on this
const formattedStartDate = moment(date, "DD-MM-YYYY");
const endDate = formattedStartDate.add(28, "days");
    ...
    sourceWorkbook.eachSheet((sourceSheet) => {
      ...
      if (!existingSheet && sourceSheet.name != "Sales") {
        ...
        if (sourceSheet.name == "P&L Data") {
          ...
          copyLaborStandardVariance(sourceSheet, newTargetSheet);
...
function copyLaborStandardVariance(sourceSheet, newTargetSheet) {
  let tempColmunNum;
  sourceSheet.eachRow((sourceRow, rowNum) => {
    sourceRow.eachCell((cell, colNum) => {
      if (rowNum === 7) {
  ...
  console.log("storeFormulaJeUploadGcolumn:",storeFormulaJeUploadGcolumn)
  for (let i = 2; i < 249; i++) {  //store formula for JE UPLOAD G column
    let formula = `VLOOKUP(Q${i},'Labor Standard Variance'!$${getColumnAddress(tempColmunNum-1)}$2:$${getColumnAddress(tempColmunNum)}$6,2,FALSE)`;
    storeFormulaJeUploadGcolumn.push(formula);
  }

  //console.log("_____",storeFormulaJeUploadGcolumn)
  ...
  sourceSheet.eachRow((sourceRow, rowNum) => {
    ...
    sourceRow.eachCell((cell, colNum) => {
      ...
      if (colNum === 4 && rowNum == 2) {
        //copy all style and result column of k in j column
        const targetCell = targetRow.getCell(colNum);
        targetCell.value = formattedStartDate;
        targetCell.style = cell.style;
      }
  ...
  //   const targetSheet = targetWorkbook.getWorksheet(newTargetSheet);
...
async function copySales() {
  ...
  const startDate = moment("18-07-2023", "DD-MM-YYYY");
  const endDate = moment("14-08-2023", "DD-MM-YYYY");
  ...
  const newArr = dataArray.filter((row) => {
    ...
    return momentDate.isAfter(startDate) && momentDate.isBefore(endDate);
  });
